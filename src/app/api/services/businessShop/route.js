...
export async function POST(req) {
  try {
    ...
    // ✅ Set qrCodeDetails if provided
    const qrImageFile = filesToUpload.find(f => f.key === "qrCodeDetails.qrCodeImage");
    if (qrImageFile) {
      const buffer = Buffer.from(await qrImageFile.file.arrayBuffer());
      const uploadedQr = await new Promise((resolve, reject) => {
        cloudinary.uploader
          .upload_stream(
            {
              resource_type: "image",
              public_id: `businessShops/qrcode_${uuidv4()}`,
            },
            (err, result) => {
              if (err) reject(err);
              else resolve(result);
      ...
      setDeep(data, "qrCodeDetails.qrCodeImage", uploadedQr.secure_url);
    }

    // Defaults for optional qrCodeDetails fields
    setDeep(data, "qrCodeDetails.location.latitude", data.qrCodeDetails?.location?.latitude ?? null);
    setDeep(data, "qrCodeDetails.location.longitude", data.qrCodeDetails?.location?.longitude ?? null);
    setDeep(data, "qrCodeDetails.location.address", data.qrCodeDetails?.location?.address ?? "");
    setDeep(data, "qrCodeDetails.renewalDate", data.qrCodeDetails?.renewalDate ?? null);
    setDeep(data, "qrCodeDetails.status", data.qrCodeDetails?.status ?? "inactive");
    setDeep(data, "qrCodeDetails.resetPasswordToken", null);
    setDeep(data, "qrCodeDetails.resetPasswordExpires", null);
