...
import {
  FaSchool, FaMapMarkerAlt, FaUserGraduate, FaCalendarAlt, FaStar,
  FaUser, FaPhone, FaAt, FaHome, FaLock, FaUpload, FaPlusCircle, FaTimes, FaGlobe,
  FaChild, FaBook, FaClipboardList, FaFileAlt, FaEye, FaEyeSlash
} from 'react-icons/fa';
...
// --- Reusable Components (Adapted for this form) ---
...
export default function StudentInfoForm() {
  // Define all possible fields for the student info form
  const initialFields = useMemo(() => [
    // School Information Section
    { id: 'schoolName', section: 'schoolInfo', label: 'College/School Name', name: 'schoolName', type: 'text', value: '', icon: <FaSchool style={{ color: COLORS.primary }} />, isRemovable: false },
    { id: 'schoolLogo', section: 'schoolInfo', label: 'College/School Logo', name: 'logo', type: 'file', multiple: false, icon: <FaUpload style={{ color: COLORS.primary }} />, isRemovable: true, fileType: 'logo' },
    { id: 'schoolAddress', section: 'schoolInfo', label: 'College/School Address', name: 'schoolAddress', type: 'textarea', value: '', placeholder: 'Full address of the school/college', isRemovable: true },
    { id: 'schoolMap', section: 'schoolInfo', label: 'College/School Map Link', name: 'schoolMap', type: 'text', value: '', icon: <FaMapMarkerAlt style={{ color: COLORS.primary }} />, isRemovable: true, special: 'location' },

    // Student Information Section
    { id: 'studentName', section: 'studentInfo', label: 'Student Name', name: 'studentName', type: 'text', value: '', icon: <FaUserGraduate style={{ color: COLORS.primary }} />, isRemovable: false },
    { id: 'dob', section: 'studentInfo', label: 'Date of Birth', name: 'dob', type: 'date', value: '', icon: <FaCalendarAlt style={{ color: COLORS.primary }} />, isRemovable: true },
    { id: 'grade', section: 'studentInfo', label: 'Grade/Class', name: 'grade', type: 'text', value: '', icon: <FaStar style={{ color: COLORS.primary }} />, isRemovable: true },

    // Parent/Guardian Information Section
    { id: 'fatherName', section: 'parentInfo', label: "Father's Name", name: 'fatherName', type: 'text', value: '', icon: <FaUser style={{ color: COLORS.primary }} />, isRemovable: true },
    { id: 'contact', section: 'parentInfo', label: 'Contact Number', name: 'contact', type: 'tel', value: '', icon: <FaPhone style={{ color: COLORS.primary }} />, isRemovable: true, placeholder: 'e.g., 1234567890' },
    { id: 'altContact', section: 'parentInfo', label: 'Alternate Contact Number', name: 'altContact', type: 'tel', value: '', icon: <FaPhone style={{ color: COLORS.primary }} />, isRemovable: true, placeholder: 'Optional' },
    { id: 'email', section: 'parentInfo', label: 'Parent Email', name: 'email', type: 'email', value: '', icon: <FaAt style={{ color: COLORS.primary }} />, isRemovable: true, placeholder: 'parent@example.com' },

    // Home Address Details Section
    { id: 'homeAddress', section: 'homeAddress', label: 'Home Address', name: 'homeAddress', type: 'textarea', value: '', placeholder: 'Student\'s home address', isRemovable: true },
    { id: 'homeMap', section: 'homeAddress', label: 'Home Map Link', name: 'homeMap', type: 'text', value: '', icon: <FaHome style={{ color: COLORS.primary }} />, isRemovable: true, special: 'location' },
    ...
    { id: 'motherName_template', section: 'additional', label: "Mother's Name", name: 'motherName', type: 'text', value: '', icon: <FaChild style={{ color: COLORS.primary }} />, isRemovable: true, isTemplate: true },
    { id: 'favoriteSubject_template', section: 'additional', label: 'Favorite Subject', name: 'favoriteSubject', type: 'text', value: '', icon: <FaBook style={{ color: COLORS.primary }} />, isRemovable: true, isTemplate: true },
    { id: 'extracurricular_template', section: 'additional', label: 'Extracurricular Activities', name: 'extracurricular', type: 'textarea', value: '', placeholder: 'e.g., Chess Club, Robotics', icon: <FaClipboardList style={{ color: COLORS.primary }} />, isRemovable: true, isTemplate: true },
    { id: 'studentPhoto_template', section: 'additional', label: 'Student Photo', name: 'studentPhoto', type: 'file', multiple: false, icon: <FaUpload style={{ color: COLORS.primary }} />, isRemovable: true, isTemplate: true, fileType: 'studentPhoto' },
  ], []);
  ...
  const [locationStatus, setLocationStatus] = useState({ schoolMap: 'idle', homeMap: 'idle' }); // Tracks status per map field
  ...
  const handleFileChange = (id, e, multiple) => {
    ...
    // For logo preview, update the value in fields state as well
    if (id === 'schoolLogo') {
      handleFieldChange(id, file ? URL.createObjectURL(file) : null);
    } else if (id === 'studentPhoto_template') {
      // Assuming 'studentPhoto_template' refers to a unique instance when added
      // You'd need to correctly map the file to the specific instance ID here
      const fieldToUpdate = fields.find(f => f.name === 'studentPhoto' && f.id === id);
      if (fieldToUpdate) {
        handleFieldChange(id, file ? URL.createObjectURL(file) : null);
      }
    }
  ...
  const addField = (templateId) => {
    ...
    if (template) {
      ...
      const newField = {
        ...
        value: template.type === 'checkbox' ? false : '', // Reset value appropriately for new instances
        isTemplate: false
      };
  ...
  const handleSubmit = (e) => {
    ...
    // Append file data as needed (e.g., base64 encoding or references)
    // For simplicity, we are logging the URL. In a real app, you'd handle file uploads
    Object.keys(files).forEach(fileId => {
      formDataToSubmit[fileId] = files[fileId]; // Store File object or process further
    });


    console.log("Student Info Form Data:", formDataToSubmit);
      ...
      >
        <div className="rounded-xl shadow-2xl overflow-hidden" style={{ backgroundColor: 'white' }}>
          ...
          <div className="p-6 text-white" style={{ background: `linear-gradient(to right, ${COLORS.primaryDark}, ${COLORS.primary})` }}>
            ...
            >
              <FaUserGraduate className="text-3xl" style={{ color: COLORS.textLight }} />
              <h1 className="text-2xl font-bold" style={{ color: COLORS.textLight }}>Student Information QR Code Generator</h1>
            </motion.div>
            <p className="text-center mt-2" style={{ color: COLORS.primaryBg }}>Enter student details to generate a QR code</p>
          </div>
          ...
          >
            ...
            <motion.div variants={itemVariants} className="p-4 rounded-lg" style={{ backgroundColor: COLORS.primaryBg }}>
              <h2 className="text-xl font-semibold flex items-center space-x-2 mb-4" style={{ color: COLORS.textPrimary }}>
                <FaUserGraduate style={{ color: COLORS.primaryDark }} />
                <span>Student Information</span>
              </h2>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <AnimatePresence>
                  {fields.filter(f => f.section === 'studentInfo').map(field => (
                    <motion.div
                      key={field.id}
                      variants={dynamicFieldVariants}
                      initial="initial"
            ...
            <motion.div variants={itemVariants} className="p-4 rounded-lg" style={{ backgroundColor: COLORS.primaryBg }}>
              ...
              <div className="space-y-4">
                <AnimatePresence>
                  {fields.filter(f => f.section === 'homeAddress').map(field => (
                    <motion.div
                      key={field.id}
                      variants={dynamicFieldVariants}
                      initial="initial"
          ...
          <div className="mt-6 space-y-2 overflow-y-auto h-[630px] p-2" style={{ color: COLORS.textPrimary }}>
            ...
            {fields.find(f => f.name === 'logo')?.value && (
              <div className="text-center">
                <img src={fields.find(f => f.name === 'logo')?.value} alt="School Logo" className="mx-auto h-20 w-20 object-cover rounded-full" />
              </div>
            )}
            {fields.filter(f => f.section !== 'security' && f.name !== 'logo' && f.name !== 'studentPhoto' && f.name !== 'password').map(field => (
              field.value && (
                <p key={field.id}>
                  <strong>{field.label}:</strong> {field.type === 'file' ? (field.value ? 'Image Uploaded' : 'No Image') : field.value}
                </p>
              )
            ))}
            {/* Display student photo if uploaded dynamically */}
            {fields.find(f => f.name === 'studentPhoto')?.value && (
               <div className="text-center mt-4">
                 <p><strong>Student Photo:</strong></p>
                 <img src={fields.find(f => f.name === 'studentPhoto')?.value} alt="Student Photo" className="mx-auto h-24 w-24 object-cover rounded-full mt-2" />
               </div>
            )}
