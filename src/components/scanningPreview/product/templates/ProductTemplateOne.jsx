...
const ProductTemplateOne = ({ items, productData }) => {
  ...
  const currentItem = products[currentSlide];
    ...
    <div className="min-h-full w-full bg-gradient-to-br from-[#008080] to-[#00a6a6] px-4 py-6 space-y-6 rounded-lg shadow-inner">
      ...
      {products.length > 0 ? (
        <div className="relative">
          <div className="bg-white rounded-xl shadow-md p-4 space-y-2 border border-gray-200 transition-all duration-300 ease-in-out">
            ...
            {currentItem?.heading && (
              <h2 className="text-lg font-semibold text-gray-800 text-center">{currentItem.heading}</h2>
            )}
            {currentItem?.description && (
              <p className="text-gray-600 text-sm text-center">{currentItem.description}</p>
            )}
            {currentItem?.pageUrl && (
              <a
