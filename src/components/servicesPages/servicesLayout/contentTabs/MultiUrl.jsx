...
export default function MultiURLForm() {
      ...
      >
        ...
        <div className="w-full lg:w-1/3 mt-10 lg:mt-0 border-2 rounded-2xl p-4 shadow-lg max-w-xs mx-auto relative overflow-hidden"
          ...
          <div className="mt-6 space-y-2 overflow-y-auto h-[630px] p-2" style={{ color: COLORS.textPrimary }}>
            ...
            {/* Display dynamic URLs */}
            {fields.filter(f => f.type === 'url' && f.value).length > 0 && (
              <div>
                <p><strong>URLs:</strong></p>
                <ul className="list-disc pl-5 space-y-1">
                  {fields.filter(f => f.type === 'url' && f.value).map(field => (
                    <li key={field.id} className="break-all">
                      <strong>{field.label.replace(' URL', '')}:</strong> {field.value}
                    </li>
                  ))}
                </ul>
              </div>
