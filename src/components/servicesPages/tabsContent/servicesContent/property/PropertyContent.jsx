...
const PropertyContent = () => {
  const { propertyDetails, setPropertyDetails, servicesDataLoading } = useServicesContext();
  ...
  const validateField = (section, key, value) => {
    ...
    if (key === "alternateNumber") {
      if (value && !propertyDetails.basicInfo.contactNumber) {
        error = "Enter main contact first";
      } else if (value && !phoneRegex.test(value)) {
        error = "Invalid alternate number";
      }
    }

    // Price/area numeric check
    if ((key === "price" || key === "area") && value && isNaN(Number(value))) {
      error = "Must be a number";
    }

    // Required field
    if (key === "propertyName" && !value.trim()) {
      error = "Required";
    }
  ...
  const handleChange = (section, key, value) => {
    ...
    validateField(section, key, value); // Live validation
  };
  ...
  const handleImageChange = (section, key, files) => {
    ...
    if (existingSize + newSize > 30 * 1024 * 1024) {
  ...
  const fetchCurrentLocation = async () => {
    ...
    try {
      ...
      alert("Failed to fetch location. Check permissions.");
    }
  };

  const validateForm = () => {
    let valid = true;
    Object.entries(propertyDetails).forEach(([section, fields]) => {
      if (typeof fields === "object") {
        Object.entries(fields).forEach(([key, value]) => {
          const isValid = validateField(section, key, value);
  ...
  const handleInitialSubmit = (e) => {
    ...
    const allValues = Object.values(propertyDetails).flatMap((section) =>
      typeof section === "object" && !Array.isArray(section) ? Object.values(section || {}) : [section]
    );
    const hasValue = allValues.some((v) => (Array.isArray(v) ? v.length > 0 : !!v?.toString().trim()));

    if (!hasValue) return toast.error("Enter at least one field");
      ...
      <div className="space-y-6">
                ...
                {key === "galleryImages" ? (
                  <>
                    <div className="flex items-center gap-4">
                      <label className="relative cursor-pointer bg-teal-600 hover:bg-teal-700 text-white font-semibold py-2 px-6 rounded-lg">
                        Choose Files
                        <input
                          type="file"
                          accept="image/*"
                          multiple
                          ref={galleryInputRef}
                          onChange={(e) => handleImageChange(section, key, Array.from(e.target.files))}
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {galleryPreview.map((item, idx) => (
                        <div key={idx} className="relative w-24 h-24 rounded border overflow-hidden shadow-sm">
                          <img src={item.url} alt={`Preview ${idx + 1}`} className="object-cover w-full h-full" />
                          <button
                  ...
                  <div className="space-y-1">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center w-full py-2 px-3 bg-[#008080] hover:bg-[#006666] text-white text-sm rounded-lg"
                    >
        ...
        <div className="relative w-full">
          <input
            ...
            onChange={(e) => setPropertyDetails((prev) => ({ ...prev, password: e.target.value }))}
        ...
        <div className="flex justify-center items-center pt-6">
          <button
            ...
            className="font-bold px-4 cursor-pointer bg-[#008080] text-white py-2 rounded text-lg"
          >
            Next →
          </button>
        </div>
