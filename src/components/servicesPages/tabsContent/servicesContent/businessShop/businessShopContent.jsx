...
//     e.preventDefault();
//     if (!formData.businessInfo.general.businessName) {
...
//         <NFCModal />
//       </div>
//       <button onClick={handleSubmit} className="w-full py-2 cursor-pointer bg-[#008080] text-white font-semibold rounded hover:bg-[#006666] transition">
//         Submit
//       </button>
//     </>
//   );
...
// // src/components/BusinessShop/BusinessShopContent.jsx
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {[1, 2, 3, 4].map((templateNum) => (
                ...
                >
                  <img
                    src={`/images/templates/businessShop${templateNum}.webp`}
          ...
          <div className="space-y-5">
            {['owner','phone', 'altPhone', 'email'].map((field) => (
              <input
                ...
                placeholder={field === 'altPhone' ? 'Alternate Phone Number' : field.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}
                className="w-full px-5 py-3 border border-gray-300 rounded-lg text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-teal-200 focus:border-teal-500 transition-all duration-200"
                value={businessInfo.contact[field] || ""}
                onChange={(e) =>
                  handleChange("businessInfo", "contact", field, e.target.value)
                }
              />
            ))}
            <div className="relative">
